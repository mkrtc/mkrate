...
5. The durable, secret-free A5 saga journal and startup recovery landed in `85ceecf` and passed independent security/crash audit.
...
A5 is implemented and independently accepted at `85ceecf`. The durable, secret-free saga covers:
...
This ADR does not declare release readiness. The corrected A0 artifact (`3b3512e`) and A5 implementation (`85ceecf`) are independently accepted. Remaining closure requires:
